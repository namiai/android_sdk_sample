...
class NamiLocalStorage private constructor(private val context: Context) {
    ...
    companion object {
        private const val namiPreferenceName = "sd_ui_extensions_sample_data_store_preference"
        ...
        val LIST_PAIRED_DEVICES = stringSetPreferencesKey("sd_ui_extensions_list_paired_devices")

        val LIST_THREAD_NETWORK_CREDENTIALS =
            stringSetPreferencesKey("sd_ui_extensions_list_thread_network_credentials")

        val LIST_SAVED_WIFI_NETWORK =
            stringSetPreferencesKey("sd_ui_extensions_list_saved_wifi_network")
        val LIST_BSSID_WIFI_NETWORK =
            stringSetPreferencesKey("sd_ui_extensions_list_bssid_wifi_network")
        val CUSTOMER_ACCESS_TOKEN =
            stringPreferencesKey("sd_ui_extensions_customer_access_token")
        val CUSTOMER_REFRESH_TOKEN =
            stringPreferencesKey("sd_ui_extensions_customer_refresh_token")
        val CUSTOMER_EXPIRES_AT =
            stringPreferencesKey("sd_ui_extensions_customer_expires_at")
        val CURRENT_PLACE_ID = intPreferencesKey("sd_ui_extensions_current_place_id")
    ...
    val customerAccessToken: Flow<NamiAccessToken?> = context.dataStore.data.catch { exception ->
        if (exception is IOException) {
            emit(emptyPreferences())
        } else {
            throw exception
